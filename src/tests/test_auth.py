...
async def test_access_protected_route_with_valid_token(
    ...
    assert protected_response.json()['username'] == TEST_USERNAME.lower()


@pytest.mark.asyncio
async def test_access_protected_route_with_not_valid_token(
    client: AsyncClient, initial_user
):
...
async def test_refresh_token_invalid_cookie(client: AsyncClient):
    ...
    client.cookies.set('refreshToken', 'invalidToken')
    response = await client.post('/auth/refresh')
...
async def test_refresh_token_expired_cookie(client: AsyncClient, initial_user):
    ...
    client.cookies.set('refreshToken', expired_refresh_token)
    response = await client.post('/auth/refresh')
